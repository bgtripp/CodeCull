...
**After ALL PRs are created**, notify the CodeCull dashboard so it can send
a Slack notification to the flag maintainer.  The credentials are in the
repo's `.codecull` file:

```
source .codecull
curl -s -X POST "$CODECULL_CALLBACK_URL" -H "Authorization: Bearer $CODECULL_CALLBACK_TOKEN"
```

This is the LAST step. Run it after pushing all branches and opening all PRs.
"""
...
def create_stacked_cleanup_session(
    ...
    """Create a single Devin session that removes multiple flags as a stacked PR chain.

    *flags* is a list of dicts, each with keys: flag_key, variation, files.

    The prompt includes a final ``curl`` step that reads callback credentials
    from the target repo's ``.codecull`` file and POSTs to the dashboard,
    triggering the Phase 2 Slack notification immediately.
